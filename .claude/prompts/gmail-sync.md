...
You are running under Claude Code (the `claude` CLI). Use the connected **`claude.ai Gmail`** MCP connector (its `mcp__*Gmail*` tools — e.g. search/list/get thread) to read mail. If Gmail access is unavailable, report that clearly and make no house-page edits.
...
Search Gmail for mail about the tracked houses from roughly the last 30 days, using the
`claude.ai Gmail` connector's search tools. Run several Gmail queries and merge the results:
...
End with a concise report only. Do not include chain-of-thought. If the Gmail connector
is unavailable, say exactly: `Gmail access unavailable; no changes made.` and include any
setup hint you can see.
